...
class CADEdgeViewerVTK(io.ComfyNode):
    """
    ...
    def define_schema(cls):
        return io.Schema(
            node_id="CADEdgeViewerVTK",
            display_name="CAD Edge Viewer (VTK)",
    ...
    """,
            inputs=[
                ...
                io.Combo.Input("visualization_mode", options=["normal", "edge_type", "length"],
                               default="normal",
                               tooltip="Color mode: normal (free=red), edge_type heatmap, or length heatmap"),
                io.Int.Input("selected_edge_id", default=-1, min=-1, max=100000,
                             tooltip="Currently selected edge ID (click edge in viewer to update)"),
            ],
            outputs=[
                io.Custom("CAD_MODEL").Output(display_name="cad_model"),
                io.String.Output(display_name="vtp_filepath"),
                io.String.Output(display_name="info"),
                io.String.Output(display_name="spline_data_file"),
                io.Int.Output(display_name="selected_edge_id"),
            ],
        )

    @classmethod
    def execute(cls, cad_model, linear_deflection=0.1, visualization_mode="normal", selected_edge_id=-1):
        ...
        for edge_idx in range(1, edge_map.Size() + 1):
            ...
            try:
                ...
                if curve_type in (GeomAbs_BSplineCurve, GeomAbs_BezierCurve):
                    edge_spline_data = _extract_spline_data(curve, curve_type)
                    if edge_spline_data:
        ...
        # Compute statistics
        length_stats = _compute_stats(length_values) if length_values else None
        ...
        return io.NodeOutput(
            cad_model, vtp_path, report, spline_json_path, selected_edge_id,
            ui={
...
def _check_planarity(geom_curve):
...
def _extract_spline_data(curve, curve_type):
    ...
    try:
        if curve_type == GeomAbs_BSplineCurve:
            ...
            # Check planarity
            planarity = _check_planarity(bspline)
            ...
            # Check planarity
            planarity = _check_planarity(bezier)
...
def _build_report(num_edges, num_vertices, num_faces,
                  bounds_min, bounds_max,
                  edge_type_counts, length_stats, free_edge_count):
    """Build text report."""
    lines = []
