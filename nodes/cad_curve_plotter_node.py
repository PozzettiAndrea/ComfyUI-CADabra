...
class CADCurvePlotter:
    ...
    def plot_curve(self, spline_data_file, edge_id, num_samples=100):
        ...
        if not spline_data_file or not os.path.exists(spline_data_file):
            return self._error_result(f"Spline data file not found: {spline_data_file}")

        try:
            with open(spline_data_file, 'r') as f:
                spline_data = json.load(f)
        except Exception as e:
            return self._error_result(f"Failed to load spline data: {e}")
        ...
        if not control_points_3d:
            return self._error_result("No control points found")

        # Project to 2D
        plane = planarity.get("plane", "XY")
        normal = planarity.get("normal", [0, 0, 1])
        control_points_2d = self._project_to_2d(control_points_3d, plane, normal)
        ...
        # Sample curve points
        curve_points_2d = self._sample_curve(
        ...
        return {"ui": {"plot_data": [result_json]}, "result": (result_json,)}
    ...
    def _project_to_2d(self, control_points_3d, plane, normal):
        ...
        for pt in control_points_3d:
            ...
            if plane == "XY":
                ...
                points_2d.append(self._rotate_to_xy(x, y, z, normal))

        return points_2d

    def _rotate_to_xy(self, x, y, z, normal):
    ...
    def _sample_curve(self, control_points, degree, knots, weights, num_samples):
        """Sample B-spline curve using De Boor algorithm."""
        if not knots or len(knots) < 2:
            # Bezier - use Bernstein polynomials
            return self._sample_bezier(control_points, degree, num_samples)
        ...
        for i in range(num_samples):
            ...
            pt = self._de_boor(t, control_points, knots, p, weights)
            curve_points.append(pt)

        return curve_points

    def _de_boor(self, t, control_points, knots, p, weights=None):
    ...
    def _sample_bezier(self, control_points, degree, num_samples):
