...
class CADEdgeAnalysis(io.ComfyNode):
    """
    ...
    def define_schema(cls):
        return io.Schema(
            node_id="CADEdgeAnalysis",
            display_name="CAD Edge Analysis",
    ...
    """,
            ...
            outputs=[
                io.Custom("CAD_MODEL").Output("cad_model"),
                io.String.Output("edge_json_filepath"),
                io.String.Output("info"),
            ],
        )

    @classmethod
    def execute(cls, cad_model, linear_deflection=0.1):
        ...
        edge_to_faces = _build_edge_to_faces_map(occ_shape, edge_map, faces_list)

        # Build vertex-to-edges mapping
        log.info("Building vertex-to-edges map...")
        vertex_to_edges = _build_vertex_to_edges_map(occ_shape, edge_map, vertex_map)
        ...
        edge_stats = _compute_stats(lengths) if lengths else None
        if edge_stats:
        ...
        return io.NodeOutput(cad_model, json_path, report, ui={"text": (report,)})
...
def _build_edge_to_faces_map(occ_shape, edge_map, faces_list):
...
def _build_vertex_to_edges_map(occ_shape, edge_map, vertex_map):
...
def _analyze_edge(edge, edge_idx, edge_to_faces, vertex_to_edges, vertex_map, deflection):
    ...
    try:
        ...
        # Curvature sampling
        edge_info["curvature"] = _sample_curvature(curve, first_param, last_param)
        ...
        edge_info["points"] = _discretize_edge(curve, deflection)
...
def _sample_curvature(curve, first_param, last_param, num_samples=10):
...
def _build_edge_report(num_edges, num_vertices, num_faces,
                       bounds_min, bounds_max,
...
class CADEdgeViewer(io.ComfyNode):
    """
    ...
    def define_schema(cls):
        return io.Schema(
            node_id="CADEdgeViewer",
            display_name="CAD Edge Viewer",
    ...
    """,
            is_output_node=True,
            inputs=[
                io.String.Input("edge_json_filepath",
                                tooltip="Path to edge analysis JSON from CADEdgeAnalysis"),
                io.Combo.Input("visualization_mode", options=["normal", "edge_type_heatmap", "length_heatmap"],
                               default="normal",
                               tooltip="Color mode: normal (free=red, shared=black), edge type heatmap, or length heatmap"),
                io.Int.Input("selected_edge_id", default=0, min=0, max=100000,
                             tooltip="Edge ID to pass to downstream nodes (set to the ID shown when you click an edge)"),
            ],
            outputs=[
                io.String.Output("edge_json_filepath"),
                io.Int.Output("selected_edge_id"),
            ],
        )

    @classmethod
    def execute(cls, edge_json_filepath, visualization_mode="normal", selected_edge_id=0):
        ...
        return io.NodeOutput(edge_json_filepath, selected_edge_id, ui={
...
class CADEdgeDetailAnalyzer(io.ComfyNode):
    """
    ...
    def define_schema(cls):
        return io.Schema(
            node_id="CADEdgeDetailAnalyzer",
            display_name="CAD Edge Detail Analyzer",
            category="CADabra/Analysis",
            description="""
    ...
    """,
            ...
            inputs=[
                io.String.Input("edge_json_filepath",
                                tooltip="Path to edge analysis JSON from CADEdgeAnalysis"),
                io.Int.Input("edge_id", default=0, min=0, max=100000,
                             tooltip="Edge ID to analyze in detail"),
                io.Combo.Input("projection_plane", options=["auto", "XY", "XZ", "YZ"],
                               default="auto",
    ...
    @classmethod
    def execute(cls, edge_json_filepath, edge_id=0, projection_plane="auto"):
        ...
        try:
            ...
            return io.NodeOutput(ui={"error": [str(e)]})
        ...
        if edge_data is None:
            return io.NodeOutput(ui={"error": [f"Edge ID {edge_id} not found"]})

        # Determine best projection plane if auto
        if projection_plane == "auto":
            projection_plane = _find_best_projection_plane(edge_data)

        # Project points to 2D
        points_2d = _project_points(edge_data.get("points", []), projection_plane)

        # Build detailed parameter info
        params = _extract_detailed_params(edge_data)
...
def _find_best_projection_plane(edge_data):
...
def _extract_detailed_params(edge_data):
